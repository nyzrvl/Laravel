          ...
          @endif
              ...
              <p>
                <small class="text-muted">
                    <p>By.<a href="/blog?author={{ $posts[0]->author->username}}" class="text-decoration-none"> {{ $posts[0]->author->name }}<a href="/blog?category={{ $posts[0]->category->slug }}" class="text-decoration-none">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans()}}
                </small>
              </p>
            ...
            <div class="col-md-4 mb-3">
              <div class="card">
                <div class="position-absolute px-3 py-2 text-white" style="background-color: rgba(0, 0, 0, 0, 7)"><a href="/blog?category={{ $post->category->slug }}" class="text-white" text-decoration-none>{{ $post->category->name}}</a></div>
                    ...
                    @else
                        <img src="https://source.unsplash.com/500x400?{{ $post->category->name }}" alt="{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
                    @endif
